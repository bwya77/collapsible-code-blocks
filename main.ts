...
export default class CollapsibleCodeBlockPlugin extends Plugin {
   ...
   private setupCodeBlock(pre: HTMLElement) {
       pre.style.setProperty('--collapsed-lines', this.settings.collapsedLines.toString());
       ...
       if (this.settings.defaultCollapsed) {
           ...
           toggleButton.innerHTML = this.settings.expandIcon;
   ...
   private createToggleButton(): HTMLElement {
       ...
       button.innerHTML = this.settings.collapseIcon;
       ...
       const toggleHandler = (e: Event) => {
           ...
           button.innerHTML = isCollapsed ? this.settings.expandIcon : this.settings.collapseIcon;
   ...
   private updateCodeBlockVisibility(pre: HTMLElement) {
    ...
    if (code) {
        code.style.overflowX = isCollapsed ? 'hidden' : 'auto';
    }
    ...
    if (markdownView?.previewMode?.containerEl) {
        ...
        // Queue multiple updates to ensure DOM changes are processed
        setTimeout(() => {
            ...
            elements.forEach(el => {
                if (isCollapsed) {
                    ...
                    if (elRect.top < preRect.bottom) {
                        el.style.marginTop = `${preRect.bottom - elRect.top + 10}px`;
                    }
                }
            });
...
class CollapsibleCodeBlockSettingsTab extends PluginSettingTab {
   ...
   display(): void {
       ...
       new Setting(containerEl)
           ...
           .setDesc('Icon to show when code block is expanded')
           .addText(text => text
               .setValue(this.plugin.settings.collapseIcon)
               .onChange(async (value) => {
                   this.plugin.settings.collapseIcon = value.trim() || DEFAULT_SETTINGS.collapseIcon;
                   await this.plugin.saveSettings();
               }));

       new Setting(containerEl)
           .setName('Expand Icon')
           .setDesc('Icon to show when code block is collapsed')
           .addText(text => text
               .setValue(this.plugin.settings.expandIcon)
               .onChange(async (value) => {
                   this.plugin.settings.expandIcon = value.trim() || DEFAULT_SETTINGS.expandIcon;
                   await this.plugin.saveSettings();
               }));
       ...
       collapsedLinesSetting.addButton(btn => {
           ...
           reloadButton.buttonEl.style.display = 'none';
       ...
       collapsedLinesSetting.addText(text => {
           text
               ...
               .onChange(async (value) => {
                   ...
                   if (reloadButton) {
                       reloadButton.buttonEl.style.display = 'block';
                   }
               });
       });
   }
